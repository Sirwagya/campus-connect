...
export function ReactionButton({
  ...
  const { reactions, myReaction, isLoading, react } = useReactions(
    targetType,
    targetId
  );
  ...
  if (isLoading) {
    return (
      <div
        className={cn(
          "h-8 w-16 bg-white/5 rounded-full animate-pulse",
          className
        )}
      />
    );
    ...
    <div className={cn("relative", className)}>
      ...
      >
        {myReaction ? (
          <>
            {(() => {
              const Icon = EMOJI_CONFIG[myReaction as Emoji].icon;
              return <Icon className="h-4 w-4" />;
            })()}
            {!compact && (
              <span className="text-sm">{EMOJI_CONFIG[myReaction].label}</span>
            )}
          </>
        ) : (
          <>
      ...
      {showPicker && (
        <div className="absolute bottom-full left-0 mb-2 z-50">
          <div className="bg-[#282828] rounded-full p-1.5 shadow-xl border border-white/10 flex gap-1">
            {(
              Object.entries(EMOJI_CONFIG) as [
                Emoji,
                (typeof EMOJI_CONFIG)[Emoji]
              ][]
            ).map(([emoji, config]) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={cn("h-5 w-5", isSelected ? "" : "text-white/80")}
                  />
                  {count > 0 && (
...
export function ReactionSummary({
  ...
  // Get unique emojis
  const uniqueEmojis = [...new Set(reactions.map((r) => r.emoji))].slice(
    0,
    3
  ) as Emoji[];
