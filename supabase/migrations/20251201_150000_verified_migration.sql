-- ================================================================
-- SAFE MIGRATION - Based on ACTUAL types/database.ts
-- Version: 20251201_150000
-- All statements are idempotent (safe to re-run)
-- ================================================================

-- ================================================================
-- PHASE 1: Add new columns to EXISTING tables
-- Based on what's NOT already in the database
-- ================================================================

-- Users: Add last_seen for presence
...
-- Events: Some columns may already exist, using IF NOT EXISTS
...
-- Spaces: Add settings (icon_url, member_count, visibility already exist)
...
-- Posts: is_pinned, like_count, comment_count already exist
ALTER TABLE public.posts
  ADD COLUMN IF NOT EXISTS is_locked boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS edited_at timestamptz;

-- Comments: Add like count and parent for threading (content column exists, not body)
...
-- Unified reactions (more flexible than post_reactions)
...
CREATE INDEX IF NOT EXISTS idx_space_members_user_status ON public.space_members(user_id, status);

-- Space messages indexes (uses author_id, not user_id)
...
CREATE INDEX IF NOT EXISTS idx_space_messages_author ON public.space_messages(author_id, created_at DESC);
...
BEGIN
  IF (TG_OP = 'INSERT') THEN
    IF NEW.target_type = 'post' THEN
      UPDATE posts SET like_count = COALESCE(like_count, 0) + 1 WHERE id = NEW.target_id;
    ELSIF NEW.target_type = 'comment' THEN
      UPDATE comments SET like_count = COALESCE(like_count, 0) + 1 WHERE id = NEW.target_id;
    END IF;
  ELSIF (TG_OP = 'DELETE') THEN
    IF OLD.target_type = 'post' THEN
      UPDATE posts SET like_count = GREATEST(0, COALESCE(like_count, 0) - 1) WHERE id = OLD.target_id;
...
DO $$ 
BEGIN 
  RAISE NOTICE '✅ Migration 20251201_150000 completed successfully!';
